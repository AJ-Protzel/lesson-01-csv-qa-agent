...
def run(df: pd.DataFrame, cases: list[Case], backend: str, model: str, effort: str,
        show_code: bool) -> int:
    passed = 0
    rows = []
    ...
    for i, case in enumerate(cases, 1):
        ...
        result = csv_agent.ask(df, case.question, backend=backend, model=model,
                               effort=effort, verbose=show_code)
...
def main() -> int:
    ...
    parser.add_argument("--model", default=csv_agent.MODEL,
                        help=f"default {csv_agent.MODEL}; claude-sonnet-5 is cheaper")
    ...
    if not cases:
        ...
        return 1
    return run(df, cases, args.backend, args.model, args.effort, args.show_code)
